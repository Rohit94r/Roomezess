...
export default function BuySellPage() {
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    try {
      await communityAPI.createPost({ type: 'sell', content });
      setContent('');
      ...
      <main className="py-8 sm:py-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">Buy & Sell</h1>
            <p className="mt-2 text-gray-600">List essentials, books, and gear for sale or request.</p>
          </div>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-3">
              ...
              <a href="/community/buy-sell" className="px-3 py-2 rounded-lg text-sm font-semibold bg-primary-600 text-white">Buy & Sell</a>
          ...
          {!checking && isVerified && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm mb-6">
              <div className="flex items-center justify-between">
                ...
                >
                  {showCreatePost ? 'Close' : 'New Listing'}
                </button>
              </div>
              {showCreatePost && (
          ...
          {!checking && isVerified && (
            <>
              {loading ? (
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm text-center text-gray-600">No listings yet.</div>
              ) : (
                <div className="space-y-4">
                  {posts.map((p) => (
                    <div key={p.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">Buy & Sell</span>
