...
export default function ProjectFridayPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const transformed = data
        ...
        .filter(
          (p: any) =>
            p.type === 'discussion' && (p.content || '').toLowerCase().includes('project')
        );
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    try {
      await communityAPI.createPost({ type: 'discussion', content });
      setContent('');
      ...
      <main className="py-8 sm:py-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">Project Friday</h1>
            <p className="mt-2 text-gray-600">Showcase weekly projects, seek collaborators, and share progress.</p>
          </div>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-3">
              ...
              <a href="/community/lost-and-found" className="px-3 py-2 rounded-lg text-sm font-semibold bg-white border border-gray-200 hover:bg-gray-100">Lost & Found</a>
              ...
              <a href="/community/project-friday" className="px-3 py-2 rounded-lg text-sm font-semibold bg-primary-600 text-white">Project Friday</a>
            </div>
          </div>
          ...
          {!checking && isVerified && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm mb-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-900">Create Project Post</h3>
                <button
                  onClick={() => setShowCreatePost(!showCreatePost)}
                  className="px-3 py-2 rounded-lg text-sm font-semibold bg-primary-600 hover:bg-primary-700 text-white"
                >
                  {showCreatePost ? 'Close' : 'New Project'}
                </button>
              </div>
              {showCreatePost && (
                <form onSubmit={handlePost} className="mt-3 space-y-3">
                  <textarea
                    placeholder="Share your project, progress, or collaboration needs..."
            ...
            <>
              {loading ? (
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm text-center text-gray-600">
                  No project posts yet. Start by creating a discussion with &quot;project&quot; in the content!
                </div>
              ) : (
                <div className="space-y-4">
                  {posts.map((p) => (
                    <div key={p.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-200">Project</span>
